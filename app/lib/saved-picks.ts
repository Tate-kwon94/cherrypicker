 ...
 *
 * 이 값으로 중복 저장을 판정하므로, 가격에 따라 변하는 값(승자, 절감액,
 * `decisionTone`)이 섞이면 같은 상품이 가격 변동마다 새 항목으로 쌓인다.
 * 반대로 너무 넓게 잡으면 사용자가 의도적으로 나눠 담은 선택이 합쳐진다.
 *
 * TODO(제품 결정): 아래 구현을 확정해 주세요.
 * 트레이드오프 —
 *   - `category:productId` 만 쓰면: 같은 상품은 항상 한 칸. 가격이 바뀌어
 *     결론이 뒤집혀도 항목이 늘지 않는다. 대신 "지난주 면세 우위"와
 *     "이번주 국내 우위"를 나란히 보관할 수 없다.
 *   - 저장 시점을 섞으면(`category:productId:날짜`): 이력이 쌓이지만
 *     같은 상품이 여러 칸을 차지해 비교함이 금방 지저분해진다.
 *   - 주류는 `productId`가 없고 취향(`taste`)으로 고르므로 별도 처리가 필요하다.
 */
export function buildIdentityKey(input: {
  category: SavedPickCategory;
  productId: string | null;
}): string | null {
  // TODO: 여기를 구현해 주세요 (5~10줄).
  // 복구할 수 없는 경우에는 반드시 null을 반환해야 합니다 —
  // null은 "모르는 항목"이라는 뜻이고, 모르는 항목끼리는 중복 판정하지 않습니다.
